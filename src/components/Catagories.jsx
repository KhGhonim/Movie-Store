...
export default function TopBooks() {
  return (
    ...
    <div className="text-center py-10">
      <h1 className="text-2xl md:text-6xl  text-[--text-color] font-bold mb-4">
        Explore Movie Categories
      </h1>
      <p className="text-sm md:text-xl text-[--text-color]">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 px-8 md:px-16">
      {categories.map((category) => (
        <Link to={category.link} key={category.name}>
          <div className="group relative overflow-hidden rounded-xl group shadow-lg bg-gray-700 transition-transform transform  hover:shadow-2xl">
            <img
              src={category.image}
              alt={`${category.name} category`}
              className="w-full h-48 object-contain transition-transform duration-300 group-hover:scale-110  bg-gray-800"
            />
